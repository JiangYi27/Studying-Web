...
- **章节随机答题**：每章随机抽 8 题测验，答错立即显示解析，答对率 ≥ 60% 即通关（并解锁下一章），未达标可重试
- **经验 / 等级系统**：通关章节获得经验奖励，升级动画 + 音效
- **知识区域**：地图上的营地节点点击即可跳转到对应章节的知识正文
...
├── knowledge/           # 知识内容统一目录
│   ├── c/               #   C 语言章节内容（Markdown）
│   ├── grammar/         #   英语语法章节内容（Markdown）
│   └── extension/       #   拓展知识文档（Word，经 /api/extensions 转换渲染）
├── data/                # 用户学习数据（已被 .gitignore 排除，不公开）
├── docs/                # 部署、重构、实战闯关说明等文档
