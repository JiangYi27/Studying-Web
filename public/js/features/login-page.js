...
(function () {
  ...
  // 标记用户是否在 hero 邮箱框输入过
  var heroEmailTyped = false;
  var heroEmailInput = document.querySelector('.login-hero-email');
  if (heroEmailInput) {
    heroEmailInput.addEventListener('input', function () {
      if (heroEmailInput.value.trim()) heroEmailTyped = true;
    });
  }

  /* ─── 视频壁纸：从 LOGIN_VIDEOS 动态生成 ─── */
  var videoContainer = document.getElementById('loginHeroVideos');
  var vtabsContainer = document.getElementById('loginHeroVtabs');
  if (!videoContainer || !vtabsContainer) return;

  var loginVideos = window.LOGIN_VIDEOS || [];
  var videos = [];
  var vtabs = [];

  loginVideos.forEach(function(v, i) {
    // 视频元素
    var vid = document.createElement('video');
    vid.className = 'login-hero-video' + (i === 0 ? ' active' : '');
    vid.src = v.src;
    vid.autoplay = true;
    vid.muted = true;
    vid.loop = true;
    vid.playsInline = true;
    vid.setAttribute('data-index', i);
    videoContainer.appendChild(vid);
    videos.push(vid);

    // 标签按钮
    var btn = document.createElement('button');
    btn.className = 'login-hero-vtab' + (i === 0 ? ' active' : '');
    btn.setAttribute('data-index', i);
    btn.textContent = v.label;
    vtabsContainer.appendChild(btn);
    vtabs.push(btn);
  });
  ...
  function showAuth() {
    ...
    // 只有用户在 hero 邮箱框真正输入过，才填入（避免覆盖浏览器记住的密码）
    if (heroEmailTyped) {
      var heroEmail = document.querySelector('.login-hero-email');
      var loginUsername = document.getElementById('loginUsername');
      if (heroEmail && heroEmail.value.trim() && loginUsername) {
        loginUsername.value = heroEmail.value.trim();
        heroEmailTyped = false; // 重置标记
      }
    }

    var loginUsername = document.getElementById('loginUsername');
    setTimeout(function () { if (loginUsername) loginUsername.focus(); }, 80);
  }
  function showHero() {
    if (!authPage || !heroPage) return;
